...
class MessagePublicationList(UniCMSListCreateAPIView):
    """
    """
    description = ""
    search_fields = ['publication__title']
    serializer_class = MessagePublicationSerializer
    ...
    def get_queryset(self):
        ...
        if newsletter_id and message_id:
            return MessagePublication.objects.filter(message__newsletter__pk=newsletter_id,
                                                     message__pk=message_id)
        return MessagePublication.objects.none()  # pragma: no cover
...
class MessagePublicationView(UniCMSCachedRetrieveUpdateDestroyAPIView):
    """
    """
    description = ""
    ...
    def get_queryset(self):
        ...
        message_publications = MessagePublication.objects.filter(message__newsletter__pk=newsletter_id,
                                                                 message__pk=message_id,
                                                                 pk=pub_id)
        return message_publications
...
class MessagePublicationFormView(APIView):

    def get(self, *args, **kwargs):
        form = MessagePublicationForm(message_id=kwargs.get('message_id'))
