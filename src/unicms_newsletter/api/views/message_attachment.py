...
class MessageAttachmentList(UniCMSListCreateAPIView):
    """
    """
    description = ""
    ...
    def get_queryset(self):
        ...
        if newsletter_id and message_id:
            return MessageAttachment.objects.filter(message__newsletter__pk=newsletter_id,
                                                    message__pk=message_id)
        return MessageAttachment.objects.none()  # pragma: no cover
...
class MessageAttachmentView(UniCMSCachedRetrieveUpdateDestroyAPIView):
    """
    """
    description = ""
    ...
    def get_queryset(self):
        ...
        attachment_id = self.kwargs['pk']
        message_attachments = MessageAttachment.objects.filter(message__newsletter__pk=newsletter_id,
                                                               message__pk=message_id,
                                                               pk=attachment_id)
...
class MessageAttachmentFormView(APIView):

    def get(self, *args, **kwargs):
        form = MessageAttachmentForm(message_id=kwargs.get('message_id'))
