...
class MessageCalendarContextList(UniCMSListCreateAPIView):
    """
    """
    description = ""
    search_fields = ['calendar_context__name']
    serializer_class = MessageCalendarContextSerializer
    ...
    def get_queryset(self):
        ...
        if newsletter_id and message_id:
            return MessageCalendarContext.objects.filter(message__newsletter__pk=newsletter_id,
                                                         message__pk=message_id)
        return MessageCalendarContext.objects.none()  # pragma: no cover
...
class MessageCalendarContextView(UniCMSCachedRetrieveUpdateDestroyAPIView):
    """
    """
    description = ""
    ...
    def get_queryset(self):
        ...
        calendar_context_id = self.kwargs['pk']
        message_calendars = MessageCalendarContext.objects.filter(message__newsletter__pk=newsletter_id,
                                                                  message__pk=message_id,
                                                                  pk=calendar_context_id)
        return message_calendars
...
class MessageCalendarContextFormView(APIView):

    def get(self, *args, **kwargs):
        form = MessageCalendarContextForm(message_id=kwargs.get('message_id'))
